...
// debug is set using     ./run debug <file>
//
...
static int ecall(u32* registers, byte* memory) {
	...
	if (n == 0) { puts("system call number error: null system call (n == 0)"); abort(); } 
		...
		if (not debug) {
			registers[10] = (u32) write((int) a0, (void*) (memory + a1), (size_t) a2); 
			registers[11] = (u32) errno; 

		} else {
			printf("write(fd=0x%08x); ", a0); fflush(stdout);
			for (u32 i = 0; i < a2; i++) {
				printf("write(0x%02hhx:'%c'); ", memory[a1 + i], memory[a1 + i]);
				fflush(stdout);
			}
			registers[10] = a2;
			registers[11] = 0;	
