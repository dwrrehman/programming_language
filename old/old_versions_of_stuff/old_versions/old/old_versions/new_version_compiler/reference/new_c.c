...
process_file:;
	...
	for (nat index = starting_index; index < text_length; index++) {
		...
		const char** list = first ? ins_spelling : names;
		const char** count = first ? ins_count : name_count;
		nat i = 0;
		for (; i < count; i++) if (not strcmp(list[i], word)) goto found;
		goto next_word;
