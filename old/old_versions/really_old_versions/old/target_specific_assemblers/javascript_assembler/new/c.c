...
enum {
	...
	and_, or_, xor_, sll, srl, slt, 
	addi, subi, muli, divi, remi, 
	andi, ori, xori, slli, srli, slti, 
	lb, lh, lw, sb, sh, sw, 
	beq, bne, blt, bge, jal, jalr, debug_, 
	dh, db, 
...
static const char* instruction_spelling[instruction_set_count] = {
	...
	"andi", "ori", "xori", "slli", "srli", "slti", 
	"lb", "lh", "lw", "sb", "sh", "sw", 
	"beq", "bne", "blt", "bge", "jal", "jalr", "debug", 
	"dh", "db", 
...
static u32 calculate_offset(nat here, nat label) {
	...
	if (label < here) {
		for (nat i = label; i < here; i++) {
			     if (ins[i].op == dh) offset -= 2;
			else if (ins[i].op == db) offset -= 1;
			else if (ins[i].op == dw) offset -= 4;
			else offset -= 8;
		}
	} else {
...
static void generate(u32 op, u32 r0, u32 r1, u32 r2) {
	emith((u16) op); 
	emith((u16) r0); 
	emith((u16) r1); 
	emith((u16) r2);
...
static void generate_b(u32 op, u32 r0, u32 r1, u32 r2, nat pc) {
	emith((u16) op); 
	emith((u16) calculate_offset(pc, registers[r0]));
	emith((u16) r1);
	emith((u16) r2);
}
...
generate_ins:
	for (nat i = 0; i < ins_count; i++) {
		...
		u32* a = ins[i].arguments;
		     ...
		     if (op == db)	emitb((u8) a[0]);
		else if (op == dh)	emith((u16)a[0]);
		else if (op == dw)	emitw((u32)a[0]);
		else if (op == ecall)   generate(0,  a[0], a[1], a[2]);
		else if (op == add)     generate(2,  a[0], a[1], a[2]);
		else if (op == sub)     generate(3,  a[0], a[1], a[2]);
		else if (op == mul)     generate(4,  a[0], a[1], a[2]);
		else if (op == div_)    generate(5,  a[0], a[1], a[2]);
		else if (op == rem)     generate(6,  a[0], a[1], a[2]);
		else if (op == and_)    generate(7,  a[0], a[1], a[2]);
		else if (op == or_)     generate(8,  a[0], a[1], a[2]);
		else if (op == xor_)    generate(9,  a[0], a[1], a[2]);
		else if (op == sll)     generate(10, a[0], a[1], a[2]);
		else if (op == srl)     generate(11, a[0], a[1], a[2]);
		else if (op == slt)     generate(12, a[0], a[1], a[2]);

		else if (op == addi)     generate(13, a[0], a[1], a[2]);
		else if (op == subi)     generate(14, a[0], a[1], a[2]);
		else if (op == muli)     generate(15, a[0], a[1], a[2]);
		else if (op == divi)     generate(16, a[0], a[1], a[2]);
		else if (op == remi)     generate(17, a[0], a[1], a[2]);
		else if (op == andi)     generate(18, a[0], a[1], a[2]);
		else if (op == ori)      generate(19, a[0], a[1], a[2]);
		else if (op == xori)     generate(20, a[0], a[1], a[2]);
		else if (op == slli)     generate(21, a[0], a[1], a[2]);
		else if (op == srli)     generate(22, a[0], a[1], a[2]);
		else if (op == slti)     generate(23, a[0], a[1], a[2]);

		else if (op == lb)       generate(24,  a[0], a[1], a[2]);
		else if (op == lh)       generate(25,  a[0], a[1], a[2]);
		else if (op == lw)       generate(26,  a[0], a[1], a[2]);
		else if (op == sb)       generate(27,  a[0], a[1], a[2]);
		else if (op == sh)       generate(28,  a[0], a[1], a[2]);
		else if (op == sw)       generate(29,  a[0], a[1], a[2]);

		else if (op == beq)       generate_b(30,  a[0], a[1], a[2], i);
		else if (op == bne)       generate_b(31,  a[0], a[1], a[2], i);
		else if (op == blt)       generate_b(32,  a[0], a[1], a[2], i);
		else if (op == bge)       generate_b(33,  a[0], a[1], a[2], i);
		else if (op == jal)       generate_b(34,  a[0], a[1], a[2], i);
		else if (op == jalr)      generate_b(35,  a[0], a[1], a[2], i);

		else if (op == debug_)      generate(40,  a[0], a[1], a[2]);
	...
	fprintf(outfile, "// autogenerated by my assembler\n\n");
	fprintf(outfile, "export const executable = [");
	for (nat i = 0; i < byte_count; i++) {
		if (i % 16 == 0) fprintf(outfile, "\n\t");
		fprintf(outfile, "0x%02hhx,", bytes[i]);
	}
	fprintf(outfile, "\n];\n");
	fclose(outfile);

	puts("\n-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
	system("cat program.js");
	puts("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n");
}
...
// autogenerated by my assembler

export const executable = [
	0x0d,0x00,0x09,0x00,0x09,0x00,0x09,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
export const myprogram = [
	...
	40, 0, 0, 0,
		...
		if (op == 0) { }
		else if (op == 1)  { if (await ecall()) return; }
		else if (op == 2)  registers[r0] = registers[r1] + registers[r2];
		else if (op == 3)  registers[r0] = registers[r1] - registers[r2];
		else if (op == 4)  registers[r0] = registers[r1] * registers[r2];
		else if (op == 5)  registers[r0] = registers[r1] / registers[r2];
		else if (op == 6)  registers[r0] = registers[r1] % registers[r2];
		else if (op == 7)  registers[r0] = registers[r1] & registers[r2];
		else if (op == 8)  registers[r0] = registers[r1] | registers[r2];
		else if (op == 9)  registers[r0] = registers[r1] ^ registers[r2];
		else if (op == 10) registers[r0] = registers[r1] < registers[r2];
		else if (op == 11) registers[r0] = registers[r1] << registers[r2];
		else if (op == 12) registers[r0] = registers[r1] >> registers[r2];
		else if (op == 13) registers[r0] = r1;
