...
static const char* keywords[] = {
	"do", "while", "repeat", "if", "then", "else", "eof", "sp", 
};
...
enum token_type {
	...
	do_token, while_token, repeat_token, if_token, then_token, else_token, eof_token, 
	less_token, colon_token, at_token, pound_token, open_curl_token, 
	close_curl_token, open_paren_token, close_paren_token, equal_token,
	plus_token, minus_token, star_token, slash_token, percent_token, tilde_token,
	token_type_count
};
...
struct token {
	nat type, value, location;
	const char* name;
};

struct tree_node {
	nat type, start, end, count;
...
static nat lex(struct token** given_tokens, char* text, nat length) {
	...
	for (nat i = 0; i < length; ) {
		...
		if (is_operator) {
			...
			//printf("operator: advancing %d characters...\n", 1);
			i++; goto next_char;
		} 
		...
		for (nat k = 0; k < keyword_count; k++) {
			...
			if (not strncmp(text + i, keywords[k], keyword_length) and 
			    (i + keyword_length >= length or is_delimiter(text[i + keyword_length]))
			) {
				puts("pushing keyword token...");
				//getchar();
				tokens = realloc(tokens, sizeof(struct token) * (token_count + 1));
				tokens[token_count++] = (struct token) { .type = nat_token + k, .location = i };
				//printf("keyword: advancing %llu characters...\n", keyword_length);
				i += keyword_length;
				goto next_char;
			}
		}
