...
enum all_output_formats { debug_output_only, macho_executable, macho_object, elf_executable, elf_object, ti_txt_executable, hex_txt_executable, };
...
static void print_instruction_window_around(
	...
	for (int64_t i = -window_width; i < window_width; i++) {
		...
		printf("  %s%4llu │ ", 
			not i and ins[here].visited ? 
			"\033[32;1m•\033[0m\033[48;5;238m"
			: (ins[here].visited ? "\033[32;1m•\033[0m" : " "), 
			here
		);
		if (not i and ins[here].visited) printf("\033[48;5;238m");
...
static nat calculate_offset(nat* length, nat here, nat target) {
	...
	if (target > here) {
		for (nat i = here; i < target; i++) {
			if (i >= ins_count) {
				printf("fatal error: calculate_offset(lengths, %lld, %lld): [ins_count = %llu]: tried to index %lld into lengths, aborting..\n", here, target, ins_count, i);
				abort();			
			}
			offset += length[i];
		}
...
process_file:;
	...
	for (nat i = 0; i < ins_count; i++) {
		...
		if (op >= set and op <= sd and i1) {
			...
			if (op == set or op == and_ or op == or_) ins[i].visited = 0;
...
arm64_instruction_selection:;
	...
	for (nat i = 0; i < ins_count; i++) {
		...
		if (op == set) {
			...
			new.args[5] = 0; // ????sf???
			mi[mi_count++] = new;
			ins[i].visited = 1; ins[b].visited = 1; ins[c].visited = 1;
			continue;
		} addsrlsl_bail:
...
finished_generation:;
	...
	if (output_format == hex_txt_executable) goto generate_hex_output;
...
generate_hex_output:;
	...
	printf("// executable autogenerated by my compiler\n");
