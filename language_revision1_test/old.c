...
enum op_code {
	op_nop, op_add, op_sub, op_xor, op_or, op_and, 
	op_sll, op_srl, op_sra, op_mul, op_div, op_rem, 
	op_slt, op_sltu, op_blt, op_bge, op_bne, op_beq, 
	op_bltu, op_bgeu, op_jal, op_jalr,
	op_slli, op_srli, op_srai, op_slti, op_sltiu, op_addi, op_xori, op_andi, op_ori,
...
static void parse(nat word_begin) {
	...
	else if (equal(w, "include_file_txt")) {  // TODO: go into a state where the next word is the filename, like how literals work.
...
top:;
	struct file_frame* tos = file_stack + file_stack_count - 1;
	...
	/*s:*/	while (tos->F_index < tos->file_length and isspace(tos->file[tos->F_index])) tos->F_index++; 
