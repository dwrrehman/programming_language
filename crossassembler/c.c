/*
	...
	use a translation layer to translate the risc-v ISA instructionsd
...
static const bool preserve_existing_objectfile = true;
static const bool preserve_existing_executable = true;
...
static void make_macho_object_file(const char* object_filename) {
...
int main(int argc, const char** argv) {
	...
	if (argc != 6 or strcmp(argv[2], "-o") or strcmp(argv[4], "-e")) 
		exit(puts("asm: \033[31;1merror:\033[0m usage: ./run <source.s> -o <objectfile.o> -e <executable>"));
...
generate_ins:
	...
	if (architecture == no_runtime) {
		if (ins_count) abort(); // ERROR: no_runtime allowed

	} else if (architecture == rv32 or architecture == rv64) {
	...
	const char* object_filename = argv[3];
	const char* executable_filename = argv[5];

	if (preserve_existing_objectfile and not access(object_filename, F_OK)) {
	...
	if (output_format == print_binary) 
		...
		make_macho_object_file(object_filename);
	else {
	...
	if (output_format == elf_executable or output_format == macho_executable) {
		if (preserve_existing_executable and not access(executable_filename, F_OK)) {
			perror("asm: executable_file: file exists"); 
