...
{ 	const char* filename = argv[1];
	...
	parse_numeric_literal:;
		...
		else if (op == target) target_arch = args[0];
		else if (op == format) output_format = args[0];
		else if (op == stacksize) stack_size = args[0];
		else {
	...
	while (stack_count) {
		...
		for (nat var = 0; var < var_count; var++) {
			...
			for (nat p = 0; p < pred_count; p++) {
				...
				if (ct_t == 0) { if (dd) { puts("future set to 0! (rt!)"); }  future_type = 0; first_ct = 0; } 
				else if (first_ct) { 
					if (dd) { puts("first! future set to 1! (ct!)"); } 
					future_type = 1;
					future_value = ct_v; 
					first_ct = 0;

				} else if (future_value != ct_v) { 
					if (dd) { puts("value mismatch! (rt)");  } 
					future_type = 0;
				}
	...
	for (nat i = 0; i < ins_count; i++) {
		...
		for (nat a = 0; a < arity[op]; a++) {
			...
			if (register_index[ins[i].args[a]] != (nat) -1) {
				...
				if (debug) printf("found a compiletime variable "
...
riscv_instruction_selection:;
	...
	/*
		...
		for (nat this = 0; this < 4; this++) {
			if (op == op_A[this] and i1 and not arg1) {
				new = (struct instruction) { b_type, 0xB, 0,  { 0x63, op_B1[this], arg0, 0, arg2,   0,0,0 } };
				goto r5_push_single_mi;
			}
			if (op == op_A[this] and i0 and not arg0) {
				new = (struct instruction) { b_type, 0x7, 0,  { 0x63, op_B1[this], 0, arg1, arg2,   0,0,0 } };
				goto r5_push_single_mi;
			}
			if (op == op_A[this]) {
				if (imm) {
				...
				new = (struct instruction) { b_type, 0x3, 0, {  0x63, op_B1[this], arg0, arg1, arg2,   0,0,0 } };
				goto r5_push_single_mi;	
			} 
		}}
		...
		*/
		...
		/*else if (op == set and not imm and is_label[arg1]) {   
			...
			new = (struct instruction) { u_type, 0x5, 0,  { 0x17, arg0, arg1, 0x42,  0,0,0,0 } };
			mi[mi_count++] = new;
		...
		/*else if (op == stb or op == sth or op == stw) {
			...
			if (op == stb) {
				...
				new = (struct instruction) { s_type, 0xf3, 0,   { 0x23, 0x001, arg0, arg1, 0x00000, 0,0,0 } };

			} else if (op == stw) {
				new = (struct instruction) { s_type, 0xf3, 0,   { 0x23, 0x010, arg0, arg1, 0x00000, 0,0,0 } };
		...
		/*else if ((op == ldb or op == ldh or op == ldw) and is_label[arg1]) {
			...
			new = (struct instruction) { i_type, 0x15, 0,   { 0x03, arg0, n, arg0, arg1,   0x42, 0,0 } };
			goto r5_push_single_mi;
		}

		else if (op == ldb or op == ldh or op == ldw) {
			nat n = 0;
			     if (op == ldb) n = 4;
			else if (op == ldh) n = 5;
			else if (op == ldw) n = 6;
	...
	for (nat var = 0; var < var_count; var++) {
		...
		if (is_label[var]) continue; // optional...
...
if (target_arch != riscv_arch) goto skip_branch_imm_replace;
	...
	for (nat i = 0; i < ins_count; i++) {
		...
		if (not (op == lt or op == ge or op == ne or op == eq or op) continue;
...
*/
		...
		//	new = (struct instruction) { i_type, 0x15, 0,   { 0x13, arg0, 0, arg0, arg1,0,  0,0 } };
		//	goto r5_push_single_mi;
		}

		//else if (op == sub and imm) { // sub d #k -> addi d d -k
		//	nat k = (-arg1) & 0xFFF;
		//	new = (struct instruction) { i_type, 0x15, 0,   { 0x13, arg0, 0, arg0, k, 0,  0,0 } };
		//	goto r5_push_single_mi;
		//}
