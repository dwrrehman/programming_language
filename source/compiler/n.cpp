...
static t n(L& l, F& f) {
    ...
    e l={};L S=s;t t=n(s,f);while(t.v&&t.v!=')'){
        if(t.v==1||!strchr("()",t.v))l.s.push_back({t});else if(t.v=='('){
    ...
    std::string o = ""; N i = 0; for(r s:E[g.i].s) if(s.t.v==1)o+="\""+std::string(s.t.s)+"\"";
    else if(s.t.v)o+=s.t.v;else o+="("+sr(g.a.size()?g.a[i++]:s,E)+")";
...
static r ra(const e&g,const r&t,N&i,N&b,N d,N m,W&E,V&S,V&I,const F&f) {
    ...
    N si=i;V Z=S;for(N s:Z.back()){ b=std::max(i,b);i=si;S=Z;std::vector<r> A={};
        if(ne(E[s].t,t,S.back()))goto c; for(N j = 0;j<E[s].s.size();j++){
            ...
            if(!E[s].s[j].t.v) { r a = ra(g,E[E[s].s[j].i].t,i,b,d+1,m,E,S,I,f);
    ...
    for (long i=1;i<ac;i++) {
        if(av[i][0]=='-') {
            ...
            else if (c=='d'&&i+1<ac)M=atol(av[++i]);
            else if (strchr("ocis", c)&&i+1<ac) { o = c; en = av[++i]; }
            ...
            const char* ex = strrchr(av[i], '.');if(ex&&!strcmp(ex, ".n")) {
                ...
                std::vector<E>E{{}};V S{{}},I(_total,std::vector<N>{});for(N i=__;i<_total;i++)I[i].push_back(i);
