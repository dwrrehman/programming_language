...
struct token { char value = 0; const char* string = ""; size_t line = 0; size_t column = 0; };
struct lexing_state { size_t at = 0; char state = none; size_t line = 0; size_t column = 0; token t = {}; };
struct resolved { size_t index = 0; std::vector<resolved> args = {}; bool error = false; };
...
static inline token next(lexing_state& l, file& file) {
    for (auto& at = l.at; at < file.length; at++) {
        char c = file.text[at];
        if (c == '\"' and !l.state) l.t = {l.state = string, file.text + at + 1, l.line, l.column};
        else if (!isspace(c) and !l.state) { at++; return l.t = {c, "", l.line, l.column++}; }
        else if (c == '\"' and l.state == string) { l.state = none; l.column++; file.text[at++] = 0; return l.t; }
        if (c == '\n') { l.line++; l.column = 1; } else l.column++;
    } if (l.state == string) { printf("n3zqx2l: %s:%ld:%ld: error: unterminated string\n\n", file.name, l.line, l.column); exit(1); } return l.t = {none, "", l.line, l.column};
...
static inline std::string expression_to_string(const expression& given, const std::vector<entry>& entries, long begin = 0, long end = -1, std::vector<resolved> args = {}) {
    ...
    for (const auto& symbol : given.sub) {
        ...
        else if (symbol.literal.value == string) result += "\"" + std::string(symbol.literal.string) + "\"";
...
//    if (expr.name.size()) { prep(depth); std::cout << "expr = " << expression_to_string(expr.name.front(), entries) << "\n"; }
    std::cout << "\n";
    long i = 0;
    ...
    for (long i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            if (ext && !strcmp(ext, ".n")) {
                ...
                if (llvm::Linker::linkModules(*module, generate(resolve(parse(state, file), {_undefined}, entries, stack, intrinsics, file, max_depth), entries, stack, intrinsics, file, context, no_files))) exit(1);
