...
static inline token next(const file& file, lexing_state& lex) {
    ...
    while (lex.index < (long) file.text.size() - 1) {
        ...
        if (is_identifier(c) and not is_identifier(n) and state == lex_type::none) { token = { token_type::identifier, std::string(1, c), lex.line, lex.column}; state = lex_type::none; lex.column++; at++; return token; }
        else if (c == '\"' and state == lex_type::none) { token = { token_type::string, "", lex.line, lex.column }; state = lex_type::string; }
        else if (c == '`' and state == lex_type::none) { token = { token_type::llvm, "", lex.line, lex.column }; state = lex_type::llvm_string; }
        else if (is_identifier(c) and state == lex_type::none) { token = { token_type::identifier, std::string(1, c), lex.line, lex.column }; state = lex_type::identifier; }
            ...
            lex.column++; at++;
        } else if ((c == '\"' and state == lex_type::string) or (c == '`' and state == lex_type::llvm_string)) { state = lex_type::none; lex.column++; at++; return token; }
        else if (is_identifier(c) and not is_identifier(n) and state == lex_type::identifier) { token.value += c; state = lex_type::none; lex.column++; at++; return token; }
            ...
            token = { token_type::operator_, std::string(1, c), lex.line, lex.column };
