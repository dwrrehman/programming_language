...
const bool debug=true;enum{_o,_i,_s,_n,_j0,_j1,_j,_a0,_a,_d0,_d1,_d,_C};
typedef size_t N;typedef std::vector<std::vector<N>> V; struct F{const char*n=0;char* t=0;N l=0;};
struct L{N i=0;N s=0;N l=0;N c=0;};struct t{N v=0;const char*s=0;N l=0;N c=0;};struct e{t t={};
std::vector<e> s={};N e=0;};struct r{N i=0;std::vector<r>a={};t t={};N e=0;};struct E{std::vector<r>
s={};r t={};r d={};};typedef std::vector<E>W;static t n(L&l,F&f){t t{};for(N&i=l.i;i<f.l;i++) {
N c=f.t[i];if(c=='\"'&&!l.s)t={l.s=1,f.t+i+1,l.l,l.c};else if(!isspace(c)&&!l.s){i++;return
{c,"",l.l,l.c++};}else if(c=='\"'&&l.s){l.s=0;l.c++;f.t[i++]=0;return t;}if(c==10){l.l++;l.c=1;}
else l.c++;}if(l.s)printf("n3zqx2l: %s:%ld:%ld: error: expected \"\n\n",f.n,l.l,l.c);return t;
}static e p(L&s,F&f){e l={};L S=s;t t=n(s,f);while(t.v&&t.v!=')'){if(t.v==1||!strchr("()",t.v))
l.s.push_back({t});else if(t.v=='('){e e=p(s,f);if(n(s,f).v!=')')printf("n3zqx2l: %s:%ld:%ld: error: expected )\n\n",f.n,t.l,t.c);
l.s.push_back(e);t.v=0;l.s.back().t=t;}S=s;t=n(s,f);}s=S;return l;}static std::string sr(const r&g,const W&E)
{std::string o="";N i=0;for(r s:E[g.i].s)if(s.t.v==1)o+="\""+std::string(s.t.s)+"\"";else if(s.t.v)o+=s.t.v;
else o+="("+sr(g.a.size()?g.a[i++]:s,E)+")";if(E[g.i].t.i)o+=" "+sr(E[g.i].t, E);return o;}
static void d(const std::vector<r>&s,const r&t,const r&d,W&E,V&S){E.push_back({s,t,d});
S.back().insert(std::upper_bound(S.back().begin(),S.back().end(),E.size()-1,[E](N ai,N bi){
if(E[ai].s.size()&&!E[ai].s[0].t.v!=(E[bi].s.size()&&!E[bi].s[0].t.v))return E[ai].s.size()&&!E[ai].s[0].t.v;
return E[ai].s.size()>E[bi].s.size();}),E.size()-1);}static N in(N c,N t,const V&I){return
std::find(I[c].begin(),I[c].end(),t)!=I[c].end();}static N ne(const r&a,const r&b,const V&S){
if(std::find(S.back().begin(),S.back().end(),a.i)==S.back().end())return 0;if(a.i!=b.i||a.a.size()!=b.a.size())
return 1;for(N i=0;i<a.a.size();i++)if(ne(a.a[i],b.a[i],S))return 1;return 0;}static r R(const e&,const r&,W&,V&,V&,const F&,N);
static r O(const char*n,const r&t,W&E,V&S,V&I,N m) {FILE*w=fopen(n,"r");if(!w){printf("n: %s: error: %s\n",n,strerror(errno));
return{0,{},{},1};}fseek(w,0,2);N l=ftell(w);char*T=(char*)calloc(l+1,sizeof(char));fseek(w,0,0);
fread(T,sizeof(char),l,w);fclose(w);F f={n,T,l};L L{0,0,1,1};return R(p(L,f),t,E,S,I,f,m);}static r u
(const e&g,const r&t,N&i,N&b,N D,N m,W&E,V&S,V&I,const F&f) {if(D>m)return{0,{},{},1};if(i<g.s.size()&&!g.s[i].t.v)
return R(g.s[i++],t,E,S,I,f,m);if(i<g.s.size()&&in(_s,t.i,I))return{t.i,{},g.s[i++].t};N si=i;V Z=S;
for(N s:Z.back()){b=fmax(i,b);i=si;S=Z;std::vector<r> A={};if(ne(E[s].t,t,S))goto c;for(N j=0;j<E[s].s.size();j++){             // ||ne(E[s].t,t,S) ///TODO: CURRENT ERROR RIGHT HERE!
if(i>=g.s.size()){if(A.size()&&j==1)return A[0];else goto c;}if(!E[s].s[j].t.v){r a=u(g,E[E[s].s[j].i].t,i,b,D+1,m,E,S,I,f);
if(a.e)goto c;A.push_back({a});}else if(E[s].s[j].t.v!=g.s[i].t.v)goto c;else i++;}if(in(_d,s,I)){
if(A[0].t.v-42<_C&&A[0].t.v>=42)I[A[0].t.v-42].push_back(E.size()); d({{0,{},{'K'}}},{_i},{},E,S);} return{s,A};c:continue;}return{0,{},{},1};
}static r R(const e&g,const r&T,W&E,V&S,V&I,const F&f,N m){N i=0,b=0;r s=u(g,T,i,b,0,m,E,S,I,f);if(i<g.s.size())s.e=1;if(s.e){
t B=b<g.s.size()?g.s[b].t:g.t;printf("n3zqx2l: %s:%ld:%ld: error: %s: unresolved %c\n",f.n,B.l,B.c,sr(T, E).c_str(),(char)B.v);}return s;
}static void sd(std::unique_ptr<llvm::Module>&m){m->setTargetTriple(llvm::sys::getDefaultTargetTriple());std::string e = "";
auto t=llvm::TargetRegistry::lookupTarget(m->getTargetTriple(),e)->createTargetMachine(m->getTargetTriple(),"generic","",{},{},{});m->setDataLayout(t->createDataLayout());}
...
}static void ee(const std::string& o, const std::string& e){std::system(std::string("ld -macosx_version_min 10.15 -lSystem -lc -o "+e+" "+o).c_str());std::remove(o.c_str());}
