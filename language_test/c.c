...
static void execute_ct_instruction(struct instruction I) {
	...
	if (I.op == op_nop) {
		printf("executing: ct_NOP[r%llu,r%llu,r%llu]\n", I._0,I._1,I._2);
		pc++;
	}
...
static void execute_instruction(struct instruction I) {
	...
	if (I.op == op_nop) {
		printf("executing: NOP[r%llu,r%llu,r%llu]\n", I._0,I._1,I._2);
		pc++;
	}
		...
		printf("executing: blt[r%llu,r%llu,label=%llu]\n", I._0,I._1,I._2);
		if (registers[I._0] < registers[I._1]) pc += ct_registers[I._2];
		else pc++;
	}
...
_: 	printf(" • ");
	...
	if (equal(string, "")) {}
		...
		for (pc = 0; pc < ins_count; ) {
			struct instruction O = instructions[i];
			
			printf("---> [%llu] CT exec ins: { operation=%s, dest=%llu, "
		...
		for (pc = 0; pc < ins_count;) {
			...
			printf("---> [%llu] RT exec ins: { operation=%s, dest=%llu, "
				"first=%llu, second=%llu }\n",
				 pc, op_code_spelling[O.op], O._0, O._1, O._2);
