...
static inline void debug_context(struct context* context) { // temp
    ...
    for (nat i = 0; i < context->type_count; i++) {
        ...
        ///TODO: print this llvm value somehow.
    }
    printf("-------------------\n\n");
...
static inline void compile(const char* filename, int8_t* text, nat length, LLVMModuleRef module, char* llvm_error) {
    ...
    const nat top_level_type = intrin_unit_type;
...
_1: 
    ...
    while (done < name.length) {
        ...
        if (c >= 256 and top + 1 < stack_size) {		   
            ...
            goto _0;
        } else if (begin >= length or c != text[begin]) goto _2;
        column++;
        begin++;
...
//             exit(2);
    ...
    exit(0);
		...
		/// its not that simple though because we actually need to evaluate things,
		///  in order to see what the reduced version compares as, not the actual expressionss. 
		/// is thaat already done by now, here?
