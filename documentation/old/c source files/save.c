...
typedef int32_t nat; // eother i16 or i32.


struct el { // doesnt exist.    we are going to do the math and casts ourselves, 
	i32 begin;
	i16 ind; // can be  i32 
	i8 type;
	i8 done;
...
int main(int argc, const char** argv) {
	...
	// NOTE must be  less than ~ 2 billion.      2 ^ 31           using i32 index.           itss just the right thing to do.

	const size_t K = 32768; 
	// NOTE: must be positive integers less than or equal to 32768.    (> 0)       2 ^ 15         using i16 index


	const size_t T = 32768;
	// note: must be a positive integer less than 128 ?       2 ^ 7            using i8 index

	const i16 S = 128; 
	// NOTE: must be a positive power of 2 less than or equal to 128.       (> 0)     2 ^ 7    using i8 index
