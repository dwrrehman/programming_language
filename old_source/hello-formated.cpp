...
static r u(const e &g, const r &t, N &i, N &b, N D, N m, W &E, V &S, V &I,
  ...
  for (N s : Z.back()) {
    ...
    if (ne(E[s].t, t, S))
      goto c;
    for (N j = 0; j < E[s].s.size();
         j++) { // ||ne(E[s].t,t,S) ///TODO: CURRENT ERROR RIGHT HERE!
      if (i >= g.s.size()) {
        if (A.size() && j == 1)
          return A[0];
        else
  ...
  for (long i = 1; i < ac; i++) {
    if (av[i][0] == '-') {
      ...
      if (ea)
        ...
        puts("usage: n [-u/-v] [-o <exe>/-c <object>/-i <ir>/-s <assembly>] "
      ...
      if (ex && !strcmp(ex, ".n")) {
        ...
        if (llvm::Linker::linkModules(
                *m, g(O(av[i], {_i}, E, S, I, M), E, S, I, av[i], C, nf)))
          continue;
