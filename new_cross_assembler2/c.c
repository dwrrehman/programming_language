...
static void print_dictionary(void) {
	...
	for (nat i = 0; i < var_count; i++) {
		if (i % 2 == 0) puts("");
		printf("[0x%5llx/%5lld]:%hhd: %24.*s :%016llx(%4lld)\t",
...
static void print_disassembly(const nat arch) {
	char string[4096] = {0};
...
static void generate_machine_instruction(nat* in, nat pc) {
		...
		if (op == emit) {
			...
			if (a0 >= (1LLU <<  7LLU)) print_error("ri: invalid 7-bit major op code", a0, pc, 1);
			if (a1 >= (1LLU <<  3LLU)) print_error("ri: invalid 3-bit minor op code", a1, pc, 2);
			if (a2 >= (1LLU <<  5LLU)) print_error("ri: invalid 5-bit destination register", a2, pc, 3);
			if (a3 >= (1LLU <<  5LLU)) print_error("ri: invalid 5-bit source register", a3, pc, 4);
			if (a4 >= (1LLU << 12LLU)) print_error("ri: invalid 12-bit immediate", a4, pc, 5);
			...
			if (a0 >= (1LLU << 7LLU)) print_error("rr: invalid 7-bit major op code", a0, pc, 1);
			if (a1 >= (1LLU << 3LLU)) print_error("rr: invalid 3-bit minor op code", a1, pc, 2);
			if (a2 >= (1LLU << 5LLU)) print_error("rr: invalid 5-bit destination register", a2, pc, 3);
			if (a3 >= (1LLU << 5LLU)) print_error("rr: invalid 5-bit source1 register", a3, pc, 4);
			if (a4 >= (1LLU << 5LLU)) print_error("rr: invalid 5-bit source2 register", a4, pc, 5);
			if (a5 >= (1LLU << 7LLU)) print_error("rr: invalid 7-bit extended op code", a5, pc, 6);
			...
			if (a1 >= (1LLU << 3LLU)) print_error("rb: invalid 3-bit condition", a1, pc, 2);
			if (a2 >= (1LLU << 5LLU)) print_error("rb: invalid 5-bit source1 register", a2, pc, 3);
			if (a3 >= (1LLU << 5LLU)) print_error("rb: invalid 5-bit source1 register", a3, pc, 4);

			//if (a4 >= (1LLU << 12LLU)) { puts("rb error"); abort(); }  //TODO: fix this
			...
			if (a0 >= (1LLU << 5LLU)) print_error("source register", a0, pc, 1);
			//if (a1 >= (1LLU << 26LLU)) print_error("label", a1, pc, 2); //TODO: fix this 
			if (a2 >= (1LLU << 1LLU)) print_error("negate bit", a2, pc, 3);
			...
			if (a0 >= (1LLU << 5LLU)) print_error("source register", a0, pc, 1);
			if (a1 >= (1LLU << 6LLU)) print_error("bit selection", a1, pc, 2);
			//if (a2 >= (1LLU << 6LLU)) print_error("label", a2, pc, 3);//TODO: fix me
			if (a3 >= (1LLU << 1LLU)) print_error("negate bit", a3, pc, 4);
			...
			if (a5 >= (1LLU <<  1LLU)) print_error("addi: is subtract bit", a5, pc, 6);
			if (a4 >= (1LLU <<  1LLU)) print_error("addi: should set flags bit", a4, pc, 5);
			if (a3 >= (1LLU <<  1LLU)) print_error("addi: shift.imm.by.12 bit", a3, pc, 4);
			if (a2 >= (1LLU << 12LLU)) print_error("addi: 12 bit immediate", a2, pc, 3);
			...
			if (a2 >= (1LLU << 5LLU)) print_error("divr: source2 register", a2, pc, 3);
			...
			if (a1 >= (1LLU << 5LLU)) print_error("orr: destination reg", a1, pc, 2);
			if (a2 >= (1LLU << 5LLU)) print_error("orr: source1 reg", a2, pc, 3);
			if (a3 >= (1LLU << 1LLU)) print_error("orr: should negate source2 bit", a3, pc, 4);
			if (a4 >= (1LLU << 5LLU)) print_error("orr: source2 reg", a4, pc, 5);
...
struct symtab_command {
	...
	memcpy(final_bytes + len, labels, sizeof labels);
	len += sizeof labels;
