...
int main(const int argc, const char** argv) {
	...
	for (int i = 0; i < length; i++) {
		if (input[i] == 46) {
			...
			buffer_count = 0;
			ins = realloc(ins, (size_t)(ins_count + 1) * sizeof(char*));
			ins[ins_count++] = strdup(buffer);
		} else if ((unsigned char) input[i] >= 33) buffer[buffer_count++] = input[i];
	}
	...
	for (int i = 0; i < ins_count; i++) {
		...
		} else if (not strcmp(ins[i], "at")) {
			printf("found an at statement! checking label register.\n");
